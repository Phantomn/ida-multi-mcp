...
from ida_multi_mcp.ida_mcp import MCP_SERVER
from ida_multi_mcp.ida_mcp.api_core import server_health, server_warmup
from ida_multi_mcp.ida_mcp.rpc import get_current_transport_session_id, tool
from ida_multi_mcp.idalib_session_manager import get_session_manager
...
def idalib_open(
    ...
    try:
        ...
        # Register headless instance in multi-mcp registry
        registry = InstanceRegistry()
        try:
            registry.register(
                pid=os.getpid(),
                port=_get_server_port(),
                idb_path=str(session.input_path),
                binary_name=session.input_path.name,
                binary_path=str(session.input_path),
                instance_type="headless",
            )
        except Exception as reg_err:
            logger.warning("Failed to register instance in registry: %s", reg_err)
...
def idalib_close(session_id: Annotated[str, "Session ID to close"]) -> dict:
    ...
    try:
        ...
        if manager.close_session(session_id):
            # Unregister from multi-mcp registry
            registry = InstanceRegistry()
            try:
                _unregister_by_pid_port(registry)
            except Exception as reg_err:
                logger.warning("Failed to unregister instance from registry: %s", reg_err)
...
def _get_server_port() -> int:
    """Return the port this server is listening on (set during main())."""
    return _SERVER_PORT


def _unregister_by_pid_port(registry: InstanceRegistry) -> None:
    """Unregister all instances matching current pid and port."""
    pid = os.getpid()
    port = _get_server_port()
    instances = registry.list_instances()
    for instance_id, info in instances.items():
        if info.get("pid") == pid and info.get("port") == port:
            registry.unregister(instance_id)


_SERVER_PORT: int = 8745
...
def main():
    ...
    # Propagate registry path via environment variable for InstanceRegistry
    if args.registry is not None:
        os.environ["IDA_MULTI_MCP_REGISTRY_PATH"] = str(args.registry)

    # Store server port for registry operations
    global _SERVER_PORT
    ...
    if args.input_path is not None:
        ...
        # Register initial binary in registry
        registry = InstanceRegistry()
        try:
            registry.register(
                pid=os.getpid(),
                ...
                binary_name=args.input_path.name,
                binary_path=str(args.input_path),
                instance_type="headless",
            )
            logger.info("Registered initial binary in registry: %s", args.input_path.name)
        except Exception as reg_err:
            logger.warning("Failed to register initial binary in registry: %s", reg_err)
    else:
        logger.info(
    ...
    def cleanup_and_exit(signum, frame):
        ...
        try:
            _unregister_by_pid_port(registry)
            logger.info("Unregistered instances from registry.")
        except Exception as reg_err:
            logger.warning("Failed to unregister from registry: %s", reg_err)
